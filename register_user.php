...
<body>
 ...
 <div class=" mb-3 ">
    ...
    <input type="text" class="form-control" name="cusname" style=" border: 2px solid #157515;" autofocus required>
  </div>
   <div class=" mb-3 ">
    <label class="form-label" style="color: white; font-size: 17px;  margin-bottom: 13px;" required>Email</label>
    <input type="text" class="form-control" name="cusEmail" style=" border: 2px solid #157515;" required>
  </div>
	<div class=" mb-3">
    <label  class="form-label" style="color: white;  font-size: 15px; margin-bottom: 15px;" required>Tel</label>
    <input type="password" class="form-control" name="cusTel"  style=" border: 2px solid #157515;" required>
  </div>
  <div class=" mb-3 ">
    <label class="form-label" style="color: white; font-size: 17px;  margin-bottom: 13px;" required>Username</label>
    <input type="text" class="form-control" name="cususer"  style=" border: 2px solid #157515;" required>
  </div>
  <div class=" mb-3">
    <label  class="form-label" style="color: white;  font-size: 15px; margin-bottom: 15px;" required>Password</label>
    <input type="password" class="form-control" name="cuspwd"  style=" border: 2px solid #157515;" required>
  </div>
	   <input type="submit" name="Submit"  value="REGISTER" class="btn btn-outline-success">
	  <a href="login_user.php"   class="btn btn-outline-danger">CANCEL </a>
    </div>
</div>
</div>
</form>
<?php
	...
	$sql = "INSERT INTO `cususer` (cus_id, cus_name, cus_user, cus_pwd, cus_Email,cus_Tel) VALUES 
	(NULL, '{$_POST['cusname']}', '{$_POST['cususer']}', md5('{$_POST[('cuspwd')]}'), '{$_POST['cusEmail']}', '{$_POST['cusTel']}' )";
	mysqli_query($conn, $sql) or die ("สมัครสมาชิกไม่สำเร็จ");
	$bid = mysqli_insert_id($conn);
			
	echo "<script>" ;
	echo "alert('SUCCESS');" ;
	echo "window.location='login_user.php';" ;
	echo "</script>" ;
}	
